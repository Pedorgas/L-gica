def calcula_idade_canina(idade_humana, porte_do_cao):
    ...
    if porte_do_cao == 'grande':
        dogao = idade_humana/ 7
        return round(dogao,2)
    elif porte_do_cao == "medio":
        dog = idade_humana/ 6
        return round(dog,2)
    else:
        dogin = idade_humana/ 5
        return round(dogin,2)
...
def calcula_aumento_salario(salario_atual):
    ...
    if salario_atual <= 724.00:
        ...
        denero = aumento + salario_atual
        return round(denero,2)
    elif salario_atual > 724.00 and salario_atual <= (724.00*2):
        aumento = (salario_atual * 15) / 100
        dinheiro = aumento + salario_atual
        return round(dinheiro,2)
    elif salario_atual > (724.00*2) and salario_atual <= (724.00*5):
        aumento = (salario_atual * 10) / 100
        denero3 = aumento + salario_atual
        return round(denero3,2)
    else:
        aumento = (salario_atual * 5) / 100
        denero4 = salario_atual + aumento
        return round(denero4,2)
...
def nota_para_conceito(nota):
    ...
    if nota <= 10.0 and nota >= 9.0:
        return "A"
    if nota <= 8.9 and nota >= 8.0:
        return "B"
    if nota <= 7.9 and nota >= 7.0:
        return "C"
    if nota <= 6.9 and nota >= 6.0:
        return "D"
    if nota <= 5.9 and nota >= 0.0:
        return "E"
